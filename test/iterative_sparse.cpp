...
int main(int argc, char **argv)
{
    ...
    try
    {
        for (std::size_t index = 1; index < argc; ++index)
        {
            ...
            for (boost::filesystem::directory_entry &file : boost::filesystem::directory_iterator(dir))
            {
                ...
                qp_container.load();
                if (qp_container.problem_.common_.objective_.positive_definite_ and "diesel.json" == qp_container.problem_.id_)
                {
    ...
    try
    {
        for (benchmark::Problem<qp::IterativeQP> &qp_container : qps)
        {
            ...
            for (qp::QP qp_problem : qp_container.problem_.instances_)
            {
                Eigen::SparseMatrix<double> ctr_sparse =
                    (has_common_ctr ? qp_container.problem_.common_.constraints_.matrix_ :
                                      qp_problem.constraints_.matrix_).sparseView();
                ...
                results.qpmad_sparse_[qp_container.problem_.id_].durations_(result_index) = timer.stop();
                const double err = (solution - qp_problem.solution_.vector_).norm();
                results.qpmad_sparse_[qp_container.problem_.id_].errors_(result_index) = err;
                if (err < 1e-9)
                {
                    // std::cout << "ok, " << timer << std::endl;
                }
    ...
    {
        ariles2::apply<ariles2::octave::Writer>("iterative_sparse.m", results);


        std::ofstream octave_script;
        octave_script.open("iterative_sparse.m", std::ofstream::out | std::ofstream::app);
        octave_script <<
R"string(
...
plot(qpmad_sparse{i}.second.durations, 'r')
legend(['qpmad, mean=', num2str(mean(qpmad{i}.second.durations))], ...
       ['qpmad_sparse, mean=', num2str(mean(qpmad_sparse{i}.second.durations))], ...
        'Location', 'northeast')
xlim([0, numel(qpmad{i}.second.durations)])
print ('iterative_sparse.png', '-dpng', '-color', '-tight', '-F:8', '-S1200,800');
)string" << std::endl;
        octave_script.close();
